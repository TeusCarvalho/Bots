...
def analisar_entregas_consolidado(df, caminho_saida):
    """Gera consolidação em múltiplas abas no Excel, incluindo motoristas únicos por Base e UF."""
    ...
    # 🚀 Motoristas únicos por Base dentro de cada UF
    motoristas_por_base = df.groupby(
        ["UF", COLUNAS["base"]]
    )[COLUNAS["motorista"]].nunique().reset_index()
    motoristas_por_base.rename(columns={COLUNAS["motorista"]: "Motoristas Únicos"}, inplace=True)

    # 🚀 Motoristas únicos totalizados por UF
    ...
    # 📊 Escrevendo todas as abas no Excel
